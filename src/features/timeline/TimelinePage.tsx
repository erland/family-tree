...
export default function TimelinePage() {
            ...
            <>
              ...
              {ev.relatedIndividuals?.map((relInd, i) => (
                <React.Fragment key={relInd.id}>
                  ...
                  <Link
                    component="button"
                    onClick={() => setSelectedId(relInd.id)}
                  >
                    {fullName(relInd)}
                  </Link>
                </React.Fragment>
  ...
  const rowsBefore = useMemo(
    () => mapEventsToRows(beforeBirth),
    [beforeBirth, mapEventsToRows]
  );
  const rowsLife = useMemo(
    () => mapEventsToRows(lifeEvents),
    [lifeEvents, mapEventsToRows]
  );
  const rowsAfter = useMemo(
    () => mapEventsToRows(afterDeath),
    [afterDeath, mapEventsToRows]
  );
  const rowsUndated = useMemo(
    () => mapEventsToRows(undated),
    [undated, mapEventsToRows]
  );
